...
fn test_encode() {
    for &(val, s) in cases::TEST_CASES.iter() {
        assert_eq!(s, bsx::encode(val, bsx::Alphabet::<58>::BITCOIN).into_string());

        assert_eq!(s.as_bytes(), &*bsx::encode(val, bsx::Alphabet::<58>::BITCOIN).into_vec());

        {
            let mut bytes = FILLER;
            assert_eq!(Ok(s.len()), bsx::encode(val, bsx::Alphabet::<58>::BITCOIN).into(&mut bytes[..]));
...

        {
            ...
            assert_eq!(Ok(s.len()), bsx::encode(val, bsx::Alphabet::<58>::BITCOIN).into(string));
            assert_eq!(s.as_bytes(), &bytes[..s.len()]);
            if !s.is_empty() {
